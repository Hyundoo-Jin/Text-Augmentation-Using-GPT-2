...
with tf.Session(config = config) as sess :
    ...
    classes = tf.placeholder(tf.float32, [args.batch_size, 9])
    ...
    h_flat = output['dense']
#     dense = tf.layers.dense(h_flat, 256)
#     dense = tf.layers.dense(dense, 32)
    dense = tf.layers.dense(h_flat, 9)
    
    class_loss = tf.reduce_mean(
                    tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits = dense, labels = classes), axis = 1)
                )
    ...
    loss = lm_loss + args.label_weight * class_loss
    ...
    ckpt = tf.train.latest_checkpoint(
        os.path.join(CHECKPOINT_DIR, 'transfer4'))
    if ckpt :
        saver.restore(sess, ckpt)
    
    
    with open('../for_new_data.pkl', 'rb') as f :
    ...
    try:
        while True:
            ...
            if args.accumulate_gradients > 1:
                ...
                cls = np.zeros((1, 9))
                for idx in (batch[1] - 10000) :
                    cls[0][idx] = 1.0
                (_, v_loss, v_lml, v_class, v_summary) = sess.run(
                    (opt_apply, loss, lm_loss, class_loss, summaries),
            ...
            print(
                '[{counter} | {time:2.2f}] loss={loss:2.2f}(lm : {lml:2.2f}, class : {cll:2.2f}) avg={avg:2.2f}'
                .format(
                    counter=counter,
