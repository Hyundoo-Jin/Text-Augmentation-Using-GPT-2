...
parser.add_argument('--dataset', metavar = 'PATH', type = str, required = True, help = 'Input file with uft-8 encoding')
...
parser.add_argument('--encoder_path', type = str, metavar = 'PATH', default = '../train.model', help = 'Sentencepiece model path')
...
parser.add_argument('--val_every', metavar = 'STEPS', type = int, default = 0, help = 'Calculate validation loss every N steps.')

parser.add_argument('--n_ctx', metavar = 'N', type = int, default = 1024, help='number of input tokens')
...
def main() :
    ...
    with tf.Session(config = config) as sess :
        ...
        with open('../long_token_paragraphs.pkl', 'rb') as f :
            chunks = pickle.load(f)

        # with open('../token_paragraphs.pkl', 'rb') as f :
        #     chunks = pickle.load(f)
        ...
        def generate_samples(enc):
            print('Generating samples...')
            context_tokens = data_sampler.sample(10)
            all_text = []
            index = 0
